...
def_wd = "/home/pwlodzimierz/ToL/git_ToL"
setwd(def_wd)
source("./aux_fun.R")
...
# for(i in seq_along(data_directories)) 
{
  ...
  array_file <- array_file[!grepl("_no_repeats_", array_file)]
  ...
  if(!replace_existing_analysis) {
    if(file.exists(paste0(assembly_name, "_repeats_filtered.csv"))) quit(save = "no", status = 1)
  }
  ...
  if (length(names(assembly)) != length(unique(names(assembly)))) {
    warning(paste0("\nWARNING: Sequence names in the fasta file are not unique \n They were appended to avoid assignment errors \n"))
    cat(paste0("\nWARNING: Sequence names in the fasta file are not unique \n They were appended to avoid assignment errors \n"))
    cat("\n", "Adjustments made: \n", paste = "")
    
    for (i in seq_along(unique(names(assembly)))) {
      if (sum(names(assembly) %in% unique(names(assembly))[i]) > 1) {
        cat("Old names:",  names(assembly)[names(assembly) == unique(names(assembly))[i]])
        cat("\n New names:",   paste0(unique(names(assembly))[i], 1:sum(names(assembly) %in% unique(names(assembly))[i])), "\n\n\n")
        names(assembly)[names(assembly) == unique(names(assembly))[i]] <- paste0(unique(names(assembly))[i], 1:sum(names(assembly) %in% unique(names(assembly))[i]))
      }
    }
  }
