...
# TODO: add CTW
# TODO: add proper name of the centromeric family
# TODO: add other details of the genome: TE/sta/holo etc
...
add_centromeric_files_to_plots <- TRUE
...
if(!replace_existing_analysis) {
  if(file.exists(paste0("global_plot_revealed_plus_edta_", assembly_name, ".png"))) {
    if(file.size(paste0("global_plot_revealed_plus_edta_", assembly_name, ".png")) != 0) {
  ...
  if(file.exists(paste0("global_plot_revealed_plus_edta_", assembly_name, ".png"))) {
    file.remove(paste0("global_plot_revealed_plus_edta_", assembly_name, ".png"))
  }
}
...
TRASH_names <- unique(unlist(strsplit(unique(centromere_data$analysed_centromeric_Trash_name), split = ";")))
scores$centormeric <- FALSE
for(jn in 1 : nrow(scores)) {
  if(scores$class[jn] %in% TRASH_names) {
    scores$centormeric[jn] = TRUE
  }
}
...
for(j in seq_along(chromosomes)) {
  ...
  scores_chromosome <- scores_chromosome[scores_chromosome$score_total >= 4 | scores_chromosome$centormeric,]
  if(nrow(scores_chromosome) > 0) {
    scores_chromosome <- rbind(scores_chromosome[scores_chromosome$centormeric,], scores_chromosome[!scores_chromosome$centormeric,])
  }
  ...
  scores_chromosome <- scores_chromosome[scores_chromosome$total_genome_bp >= 5000 | scores_chromosome$centormeric,]
  if(nrow(scores_chromosome) == 0) next
  scores_filtered <- rbind(scores_filtered, scores_chromosome)
}

classes$centromeric <- FALSE
classes$centromeric[classes$class %in% TRASH_names] <- TRUE
...
png(filename = paste0("global_plot_revealed_plus_edta_", assembly_name, ".png"), width = 3000, 
...
layout(matrix(1:(4*length(chromosomes)+1), nrow = (1 + 4*length(chromosomes)), ncol = 1), 
       widths = 1, heights = c(700, rep(c(80,300,200,200),length(chromosomes))))
...
for(j in 1 : length(chromosomes))
{
  ...
  cat(" table;")
  for(kn in seq_len(nrow(scores_chromosome_plot))) {
    if(scores_chromosome_plot$class[kn] %in% TRASH_names) {
      scores_chromosome_plot$class[kn] <- paste0("CEN_",scores_chromosome_plot$class[kn]) # TODO fix this
    }
  }
  ...
  if(add_centromeric_files_to_plots) {
    ...
    for(as in seq_along(starts)) {
      rect(starts[as], 0, ends[as], 100, col = "#00000020", border = NULL)
    }
  }
  ...
  if(add_centromeric_files_to_plots) {
    ...
    for(as in seq_along(starts)) {
      rect(starts[as], 0, ends[as], 100, col = "#00000040", border = NULL)
    }
...
system2(command = "cp", args = c(paste0("./global_plot_revealed_plus_edta_", assembly_name, ".png"), 
                                 paste0("../../../plots_repeats_classes_gc_edta_gene_landscape/global_plot_revealed_plus_edta_", assembly_name, ".png")))
