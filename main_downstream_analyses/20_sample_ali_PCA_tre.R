...
for(h in seq_len(nrow(satellites_metadata))) {
  ...
  pca_df$seqID <- sample_repeats$seqID
  
  
  write.csv(x = pca_df, file = paste0(assembly_name_short, "_", repeat_trash, "_pca_df.csv"))
  
  pca_df_shuffled <- pca_df[sample(nrow(pca_df)), ]
  
  x_limits <- quantile(pca_df_shuffled$PC1, probs = c(0.01, 0.99), na.rm = TRUE)
  y_limits <- quantile(pca_df_shuffled$PC2, probs = c(0.01, 0.99), na.rm = TRUE)
  
  p <- ggplot(pca_df_shuffled, aes(x = PC1, y = PC2, color = seqID)) +
    ...
    coord_cartesian(xlim = x_limits, ylim = y_limits) +  # Trim the plot region
    theme_minimal() +
    theme(
    ...
    # Save the plot
  ggsave(paste0(assembly_name_short, "_", repeat_trash, "_pca_satellite_repeats_colored_borders.pdf"), p, width = 12, height = 10, dpi = 300)
  ggsave(paste0("/home/pwlodzimierz/ToL/upload_files/20_PCA/", assembly_name_short, "_", repeat_trash, "_pca_satellite_repeats_colored_borders.pdf"), p, width = 12, height = 10, dpi = 300)
