...
for(species in 1:length(species.list)){
  ...
  if(length(grep(gap.i,list.files.species)) > 0){
    ...
    data.i.relevant <- data.i.relevant[with(data.i.relevant,order(TRASH.chr,TRASH.start)),]
    ...
    data.i.relevant$chr.factor <- data.i.relevant$TRASH.chr %>%
      {gsub("[^0-9]+","",.)} %>%
      as.numeric()
    data.i.relevant$chr.oddness <- mapply(function(chr) ifelse(chr%%2 == 0, "even", "odd"),data.i.relevant$chr.factor)
    
    data.i.relevant <- data.i.relevant[with(data.i.relevant,order(chr.factor,TRASH.start)),]
    
    if(plt_ == T){
      
      p.1[[length(p.1)+1]] <- ggplot(data.i.relevant) +
        geom_rect(aes(ymin = GAP.length.id-0.5, ymax = GAP.length.id+0.5, xmin = -Inf, xmax = Inf, fill = chr.oddness), color = NA) +
        scale_fill_manual(values = c("even" = "transparent", "odd" = "#ced8e2"), na.value = "transparent") +
        ...
        coord_cartesian(xlim = c(-0.5,100), ylim = ylim) +
        theme_transparent() +
        theme(legend.position = "none",
              ...
              panel.grid.major.x = element_line(colour = "grey97",linetype = "dotted"),
      ...
      p.2[[length(p.2)+1]] <- ggplot(data.i.relevant) +
        ...
        labs(title = paste0(" \n",unique(data.i.relevant$scf_name)," (n: ",gap.n.i,")\n ")) +
        guides(color = guide_legend(byrow = TRUE)) +
        coord_cartesian(xlim = c(-0.5,20), ylim = ylim) +
        theme_transparent() +
        theme(legend.position = "none",
              legend.direction = "vertical",
              legend.title = element_blank(),
      ...
      p.5[[length(p.5)+1]] <- ggplot(data.i.relevant) +
        ...
        labs(title = paste0(" \n",unique(data.i.relevant$scf_name)," (n: ",gap.n.i,")\n ")) +
        guides(color = guide_legend(byrow = TRUE)) +
        coord_cartesian(xlim = c(-0.5,25), ylim = ylim) +
        theme_transparent() +
        theme(legend.position = "none",
              legend.direction = "vertical",
              legend.title = element_blank(),
...
pdf(paste0("figure_s11_panel_C".pdf"),onefile = T, width = (length(species.list)*18)/7,height = 9)
