...
#
# Script no 11 needs to be finished before this
#
...
add_centromeric_files_to_plots <- TRUE # needs script no XX to finish before
...
if(add_centromeric_files_to_plots) {
  centromere_file <- list.files(path = "./", pattern = "_genome_metadata.csv", full.names = TRUE)
...
chromosomes_all <- chromosomes
chromosomes_lengths_all <- chromosomes_lengths

plots_number = floor(length(chromosomes_all) / 30)

for(z in 1 : plots_number) {
  chromosomes <- chromosomes_all[((z-1)*30+1) : (z*30)]
  chromosomes <- chromosomes[!is.na(chromosomes)]
  chromosomes_lengths <- chromosomes_lengths[((z-1)*30+1) : (z*30)]
  chromosomes_lengths <- chromosomes_lengths[!is.na(chromosomes_lengths)]
  
  png(filename = paste0("global_plot_plus_edta_", z, "_", assembly_name, ".png"), width = 3000, 
  ...
  system2(command = "cp", args = c(paste0("./global_plot_plus_edta_", z, "_", assembly_name, ".png"), 
                                   paste0("../../../plots_repeats_classes_gc_edta_gene_landscape/global_plot_plus_edta_", z, "_", assembly_name, ".png")))
}
